...
public class ORDSPojoGetRequestTest extends HRTestBase {
    ...
    public void regionPojoTest() {
        ...
        //Do I have any motivation to use jsonPath here? No, actually
        //Because we prepared pojo for all the properties, we dont need to use any path, so as() method is enough
        //Regions regions1 = get("/regions")
        //        .then().statusCode(200)
        //        .extract().jsonPath().getObject("items", Regions.class);


        //verify count is 4
        assertThat(regions.getCount(), is(4));

        //create empty list to store values to put the texts of the json keys
        List<String> regionNames = new ArrayList<>();
        List<Integer> regionIds = new ArrayList<>();

        //get list of regions out of regions object
        List<Region> items = regions.getItems(); //getter method you created to get 'items'

        //loop through each of the region, save their ids and names to empty list that we prepare
        for (Region region : items) {
            regionIds.add(region.getRId());           //getter
            regionNames.add(region.getRegion_name()); //getter
        }
